...
contract Ownable {
    //  TODO's
        ...
        modifier onlyOwner(){
            require(_owner==msg.sender,"if called by any account other than the owner");
            _;
...
function transferOwnership(address newOwner) public onlyOwner {
            require(newOwner== address(newOwner),"Address is real");
            _owner=newOwner;
...
contract Pausable is Ownable{
    ...
    function setter(bool pause) public onlyOwner{
        _paused=pause;
    }
     constructor()public{
         _paused=false;
    ...
    modifier whenNotPaused(){
        ...
        require(_paused==false,"constract is paused");
     ...
     modifier paused(){
        ...
        require(_paused==true,"constract is not paused");
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public onlyOwner{
        require(ownerOf(tokenId)!=address(to),"require the given address to not be the owner of the tokenId");
        
        require(isApprovedForAll(ownerOf(tokenId),to)==true,"require the msg sender to be the owner of the contract or isApprovedForAll() to be true");
    ...
    function getApproved(uint256 tokenId) public view returns (address) {
       // require( _tokenApprovals[tokenId]!=address(0),"return token approval if it exists");
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
           ...
           require(ownerOf(tokenId)==from,"require from address is the owner of the given token");
            ...
            require(to==address(to),"Not valid dddress");
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
    ...
    function setTokenURI(uint256 tokenId) internal {
            require(_exists(tokenId)==true,"require token id to exist");
            string memory tokenid=uint2str(tokenId);
            string memory tokenURI=strConcat(_baseTokenURI,tokenid);
